...
export default function CreditMeter({ used, limit, size = 'lg' }: CreditMeterProps) {
  ...
  const color = percentage > 90 ? 'bg-red-500' : percentage > 75 ? 'bg-[#00ffca]' : 'bg-emerald-500';
    ...
    return (
      <div className="flex flex-col gap-1.5 min-w-[120px]">
        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-white/30">
          ...
          <span className={percentage > 90 ? 'text-red-400' : 'text-[#00ffca]'}>{used}/{limit}</span>
        </div>
  ...
  return (
    <div className="bg-white/5 border border-white/5 p-6 rounded-xl">
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-xl font-sans italic text-white tracking-tighter">{used} <span className="text-white/20">/ {limit}</span></span>
      </div>
