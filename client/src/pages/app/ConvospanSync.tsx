...
export default function ConvospanSync() {
  ...
  if (loading && !config && !stats) {
    return <div className="flex items-center justify-center h-64 animate-pulse text-[#D4AF37] font-black uppercase tracking-widest text-xs">Initializing Protocol Node...</div>;
  }
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-serif italic text-white tracking-widest uppercase">Sync Node: ConvoSpan</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {testing ? <RefreshCw className="w-3 h-3 animate-spin" /> : <FlaskConical className="w-3 h-3 text-[#D4AF37]" />}
            Test Connectivity
          </button>
          {!isEditing && (
            <button 
              onClick={() => setIsEditing(true)}
              className="flex items-center gap-2 px-4 py-2 bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-lg text-[10px] font-black uppercase tracking-widest text-[#D4AF37] hover:bg-[#D4AF37]/20 transition-all"
            >
        ...
        <div className={`p-4 rounded-xl border flex items-center justify-between animate-slide-in-top ${testResult.ok ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-serif italic opacity-80">{testResult.ok ? `Latency: ${testResult.latency_ms}ms · Standing by for telemetry.` : `Error: ${testResult.error || `HTTP ${testResult.status}`}`}</p>
            </div>
          </div>
          ...
          <section className="bg-white/5 border border-white/5 rounded-2xl p-6 backdrop-blur-xl">
             ...
             {isEditing ? (
               <form onSubmit={handleSaveConfig} className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-lg py-2 px-3 text-xs text-white focus:outline-none focus:border-[#D4AF37]/50"
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button 
                      type="submit"
                      disabled={saving}
                      className="flex-1 bg-[#D4AF37] text-black text-[10px] font-black uppercase tracking-widest py-2 rounded-lg hover:bg-[#D4AF37]/80 disabled:opacity-50"
                    >
               ...
               <div className="space-y-4">
                  <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                    <div className="flex justify-between items-start mb-4">
                       ...
                       <span className="text-[9px] font-black text-[#D4AF37] uppercase tracking-widest">Protocol 2.0</span>
                    </div>
               ...
               <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/20 p-6 rounded-xl text-center">
                  <AlertCircle className="w-10 h-10 text-[#D4AF37]/30 mx-auto mb-4" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#D4AF37] mb-2">Sync Node Not Configured</p>
                  ...
                  <button 
                    onClick={() => setIsEditing(true)}
                    className="w-full bg-[#D4AF37] text-black text-[10px] font-black uppercase tracking-widest py-2 rounded-lg hover:bg-[#D4AF37]/80"
                  >
                    Configure Now
                  </button>
               </div>
          ...
          <section className="bg-white/5 border border-white/5 rounded-2xl p-6 backdrop-blur-xl">
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                   ...
                   <p className="text-2xl font-serif italic text-white">{stats?.totals?.success_rate_pct || '0'}%</p>
                </div>
                <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                   <p className="text-[8px] font-black uppercase tracking-widest text-white/20 mb-1">Signals Today</p>
                   <p className="text-2xl font-serif italic text-white">{stats?.totals?.today || '0'}</p>
                </div>
             </div>
          ...
          <section className="bg-white/5 border border-white/5 rounded-2xl p-6 backdrop-blur-xl">
             ...
             <div className="space-y-4">
                {stats?.by_campaign.length ? stats.by_campaign.map(camp => (
                  <div key={camp.campaign_id} className="bg-black/20 p-3 rounded-lg flex items-center justify-between group hover:border-[#D4AF37]/30 border border-transparent transition-all">
                     <div className="flex items-center gap-3">
                        <div className="w-2 h-2 rounded-full bg-[#D4AF37]/40 group-hover:bg-[#D4AF37] transition-all" />
                 ...
                 <table className="w-full text-left border-collapse">
                    <thead>
                       <tr className="bg-white/[0.02] border-b border-white/10">
                          ...
                          <th className="py-4 px-6 text-[9px] font-black uppercase tracking-widest text-[#D4AF37]">Entity Identity</th>
                    ...
                    <tbody className="divide-y divide-white/[0.02]">
                       {logs.length > 0 ? logs.map(log => (
                         <tr key={log.id} className="hover:bg-white/[0.02] transition-all group">
                            ...
                            <td className="py-4 px-6">
                               <div>
                                  ...
                                  <div className="flex gap-2 text-[8px] text-white/20 uppercase font-black tracking-widest mt-1">
                                     <span className="text-[#D4AF37]/60 group-hover:text-[#D4AF37] transition-all">{log.sector || 'METRICS'}</span>
                                     <span>·</span>
                            ...
                            <td className="py-4 px-6">
                               <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${log.triggered_by === 'auto' ? 'border border-white/5 text-white/30' : 'bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20'}`}>
                                  {log.triggered_by}
                               </span>
                            </td>
                            <td className="py-4 px-6 text-right">
                               <button 
                                 ...
                                 className="text-[9px] font-black uppercase tracking-widest text-white/20 hover:text-[#D4AF37] transition-all flex items-center gap-2 justify-end ml-auto"
                               >
