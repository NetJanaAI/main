...
export default function Reports() {
  ...
  const handleExport = async (format: 'pdf' | 'csv') => {
    if (!EXPORT_BACKEND_IMPLEMENTED) return; // guard — don't even fire the request
  ...
  return (
    <div className="flex flex-col gap-12 animate-fade-in max-w-5xl">
       <header>
          <h1 className="text-3xl font-serif italic text-white tracking-widest uppercase mb-2">Alpha Reports</h1>
       ...
       <div className="grid md:grid-cols-2 gap-8">
          <section className={`bg-white/5 border border-white/5 p-12 rounded-2xl flex flex-col justify-between backdrop-blur-3xl relative overflow-hidden group transition-all ${!EXPORT_BACKEND_IMPLEMENTED ? 'opacity-50' : 'hover:border-[#D4AF37]/20'}`}>
             <div className="absolute top-0 right-0 w-64 h-64 bg-[#D4AF37]/5 blur-3xl pointer-events-none -mr-32 -mt-32" />
             <div>
                <PieChart className="w-12 h-12 text-[#D4AF37] mb-8" strokeWidth={1.5} />
                <h3 className="text-2xl font-bold font-serif text-white uppercase italic italic-strong mb-4">Intelligence Briefing (PDF)</h3>
             ...
             <button 
               ...
               className="h-14 w-full bg-[#D4AF37] text-black text-xs font-black uppercase tracking-[3px] rounded-xl flex items-center justify-center gap-3 disabled:opacity-40 disabled:cursor-not-allowed hover:enabled:bg-[#D4AF37]/80 transition-all leading-none"
             >
                {!EXPORT_BACKEND_IMPLEMENTED
          ...
          <section className={`bg-white/5 border border-white/5 p-12 rounded-2xl flex flex-col justify-between backdrop-blur-3xl relative overflow-hidden group transition-all ${!EXPORT_BACKEND_IMPLEMENTED ? 'opacity-50' : 'hover:border-emerald-500/20'}`}>
             <div>
                ...
                <h3 className="text-2xl font-bold font-serif text-white uppercase italic italic-strong mb-4">Raw Signal Matrix (CSV)</h3>
                <p className="text-[10px] text-white/30 font-bold uppercase tracking-[2px] leading-relaxed mb-8">
       ...
       <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/10 p-12 rounded-2xl">
          <div className="flex items-center gap-3 mb-6">
             <Shield className="w-6 h-6 text-[#D4AF37]" />
             <h4 className="text-xs font-black uppercase tracking-widest text-[#D4AF37]">Alpha Compliance Notice</h4>
          </div>
          ...
          <button className="mt-8 text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-white flex items-center gap-2 transition-colors">
