...
export default function ApiManager() {
  ...
  return (
    <div className="flex flex-col gap-12 animate-fade-in">
       <header>
          <h1 className="text-3xl font-sans italic text-white tracking-widest uppercase mb-2">Registry Access Keys</h1>
       ...
       <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 w-fit">
          {[
            ...
            <button 
              ...
              className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 transition-all ${activeTab === tab.id ? 'bg-[#00ffca] text-black shadow-[0_0_20px_rgba(0,255,202,0.2)]' : 'text-white/40 hover:text-white/60 hover:bg-white/5'}`}
            >
       ...
       <div className="grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-8">
             {activeTab === 'sources' && (
                <section className="space-y-8">
                   <div className="flex justify-between items-center bg-white/5 border border-white/5 p-8 rounded-2xl">
                      ...
                      <button onClick={() => setIsAdding(!isAdding)} className="px-6 py-2.5 bg-[#00ffca] text-black text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 hover:bg-[#00ffca]/80 transition-colors">
                        <Plus className="w-4 h-4" /> {isAdding ? 'Cancel' : 'Add Integration'}
                      </button>
                   </div>

                   {isAdding && (
                     <form onSubmit={handleAddIntegration} className="bg-black/40 border border-[#00ffca]/20 p-12 rounded-2xl animate-slide-in-top space-y-8 backdrop-blur-3xl shadow-6xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-[#00ffca]/5 blur-3xl pointer-events-none -mr-32 -mt-32" />
                        
                        <div className="grid md:grid-cols-2 gap-8 relative z-10">
                           <div className="space-y-3">
                              <label className="block text-[10px] font-black uppercase tracking-[3px] text-[#00ffca]">Provider Cluster</label>
                              <select 
                                value={newCred.provider} 
                                onChange={e => setNewCred({...newCred, provider: e.target.value})}
                                className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-xs font-bold uppercase tracking-widest text-white focus:outline-none focus:border-[#00ffca]/50"
                              >
                           ...
                           <div className="space-y-3">
                              <label className="block text-[10px] font-black uppercase tracking-[3px] text-[#00ffca]">Integration Alias</label>
                              <input 
                                type="text" 
                                value={newCred.name} 
                                onChange={e => setNewCred({...newCred, name: e.target.value})}
                                placeholder="E.G. PRIMARY_SELLER_ACCOUNT" 
                                className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-xs font-bold uppercase tracking-widest text-white focus:outline-none focus:border-[#00ffca]/50 placeholder:text-white/10"
                              />
                           </div>
                        </div>
                        <div className="space-y-3 relative z-10">
                           <label className="block text-[10px] font-black uppercase tracking-[3px] text-[#00ffca]">Secure Value / API Key / Secret</label>
                           <textarea 
                             value={newCred.value} 
                             onChange={e => setNewCred({...newCred, value: e.target.value})}
                             placeholder="PASTE VERIFIED PROTOCOL TOKEN HERE..." 
                             className="w-full h-32 bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-xs font-black font-data text-white focus:outline-none focus:border-[#00ffca]/50 placeholder:text-white/10 resize-none"
                           />
                        </div>
                        <button type="submit" className="h-14 w-full bg-[#00ffca] text-black text-xs font-black uppercase tracking-[3px] rounded-xl hover:shadow-[0_0_30px_rgba(0,255,202,0.4)] transition-all leading-none">
                   ...
                   <div className="grid md:grid-cols-2 gap-6">
                      {loading ? (
                        ...
                        <div key={int.id} className="bg-white/5 border border-white/5 p-8 rounded-2xl group hover:border-[#00ffca]/40 transition-all flex flex-col justify-between backdrop-blur-md">
                           <div>
                              ...
                              <h4 className="text-xl font-bold font-sans italic text-white uppercase italic-strong leading-none mb-2">{int.credential_name}</h4>
                              <span className="text-[10px] font-black uppercase tracking-widest text-[#00ffca]">{int.provider} Protocol</span>
                           </div>
              ...
              {activeTab === 'webhooks' && (
                <section className="space-y-6">
                  ...
                  <div className="bg-white/5 border border-white/5 p-12 rounded-2xl text-center flex flex-col items-center">
                   ...
                   <h3 className="text-2xl font-bold text-white uppercase tracking-widest italic font-sans">Registry Ingest Nodes</h3>
             ...
             {activeTab === 'docs' && (
                <section className="bg-white/5 border border-white/5 p-12 rounded-2xl relative overflow-hidden backdrop-blur-3xl">
                   <div className="absolute top-0 right-0 w-64 h-64 bg-[#00ffca]/5 blur-3xl pointer-events-none -mr-32 -mt-32" />
                   <h3 className="text-xl font-bold text-white uppercase tracking-widest italic font-sans mb-8 border-b border-white/5 pb-4">Protocol Specifications v2.4</h3>
                   
                   <div className="space-y-12 relative z-10">
                      <div>
                         <h4 className="text-[11px] font-black uppercase tracking-[3px] text-[#00ffca] mb-4">Signal Ingestion Endpoint</h4>
                      ...
                      <div>
                         <h4 className="text-[11px] font-black uppercase tracking-[3px] text-[#00ffca] mb-4">Ingestion Code Example (CURL)</h4>
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
             <div className="bg-[#00ffca]/5 border border-[#00ffca]/10 p-10 rounded-2xl">
                <Zap className="w-8 h-8 text-[#00ffca] mb-6 opacity-60" />
